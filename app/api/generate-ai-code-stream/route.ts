              ...
              if (intentResponse.ok) {
                ...
                if (
                  ...
                  if (target) {
                    await sendProgress({
                      type: "status",
                      message: `✅ Found code in ${target.filePath
                        .split("/")
                        .pop()} at line ${target.lineNumber}`,
                    });
...
User request: "${prompt}"`;
          ...
          if (editContext) {
            ...
            await sendProgress({
              type: "status",
              message: `Identified edit type: ${
                editContext.editIntent?.description || "Code modification"
              }`,
            });
          } else if (!manifest) {
            console.log(
...
Provide the complete file content without any truncation. Include all necessary imports, complete all functions, and close all tags properly.`;
                ...
                const normalizedModelName = model.includes("/")
                  ? model.split("/").pop()
                  : model;

                const completionResult = await streamText({
                  model: completionClient(normalizedModelName as string),
                  messages: [
                    {
                      role: "system",
                      content:
